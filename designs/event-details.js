...
function eventDetails(event) {
  ...
  return `<span class="detail-type ${esc(event.type)}"><span class="dot"></span>${esc(text(types[event.type].label))}</span>
    ...
    ${!event.cancelled&&(event.registrationUrl||event.qr||event.registration)?`<section class="detail-section"><h3>${state.lang?'Registration':'报名入口'}</h3>${event.registrationUrl?`<a class="primary registration-button" href="${esc(event.registrationUrl)}" target="_blank" rel="noopener noreferrer">${esc(t('registration'))} ↗</a>`:event.registration?`<button class="primary registration-button" data-registration>${esc(t('registration'))} ↗</button>`:''}${event.qr?mediaImage(event.qr==='sample'?'assets/registration-qr.svg':'assets/missing-qr.png',state.lang?'Demo registration QR code — points to example.org':'演示报名二维码，指向 example.org',false):''}${event.qr?`<p class="media-caption">${state.lang?'Demo destination: example.org':'示例目标：example.org'}</p>`:''}<p class="external-note">${esc(t('external'))}</p></section>`:''}
    <p class="detail-updated">${esc(event.updatedAt ? (state.lang ? 'Updated: ' : '更新于：') + event.updatedAt : t('updated'))}</p>`;
}

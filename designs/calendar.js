...
document.body.className = ['classic','editorial','airy'].includes(theme) ? theme : 'airy';
/** @returns {any} */
...
function renderCalendar(){
  ...
  for(let w=0;w<count/7;w++){
    ...
    for(let i=0;i<7;i++){
      ...
      html+=`<div class="day${d.getMonth()!==state.month?' outside':''}${i>4?' weekend':''}${iso==='2026-09-19'?' today':''}"><button class="day-number" data-day="${iso}" aria-label="${esc(formatDate(iso,true))}" ${iso==='2026-09-19'?'aria-current="date"':''}>${d.getDate()}</button><div class="day-items" style="padding-top:${visibleLanes*26}px">${items.slice(0,cap).map(eventButton).join('')}${hiddenCount>0?`<button class="more" data-day="${iso}">${state.lang?`+${hiddenCount} more`:`还有 ${hiddenCount} 项`}</button>`:''}</div></div>`;
    }

...
管理后台的「放假与调休」页支持按单日或日期范围设置全校「放假 / 全天上课 / 上半天 / 恢复默认」。周一至周五默认全天上课，周末默认休息；仅人工指定后显示上半天，周五和假前日期也不会自动变为半天。恢复默认会清除所选日期的人工设置。已有调休上课记录视为人工指定全天。半天以浅橙底和「上半天 / Half day」标识，放假以绿色底显示，各视图独立于事件筛选，也不计入事件数量。

放假只由此页面登记，不会自动导入法定节假日；寒暑假和临时放假也需登记。事件不再提供假期类型，升级时删除原有测试假期事件，保留日期安排。样本命令仅添加休假与调休日期，不再生成假期事件。

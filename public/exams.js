...
function render(){
 ...
 $('#schedule').innerHTML=`<div class="schedule-grid" style="--day-count:${shownDays.length}">${shownDays.map(day=>`<section class="exam-day${[0,6].includes(date(day).getDay())?' weekend':''}${day===today?' today':''}"><div class="exam-day-header"><small>${new Intl.DateTimeFormat(lang?'en-GB':'zh-CN',{weekday:'short'}).format(date(day))}</small><strong>${date(day).getDate()}</strong></div>${thisWeek.filter(s=>s.date===day).map(s=>`<article class="exam-card${chosen.has(s.id)?' chosen':''}${s.cancelled?' cancelled':''}"><label><input type="checkbox" data-choice="${esc(s.id)}" ${chosen.has(s.id)?'checked':''} ${!school.user||saving?'disabled':''} aria-label="${esc(T('选择 ','Select ')+title(s))}"><button data-exam="${esc(s.id)}"><time>${s.start}–${s.end}</time><h3>${esc(title(s))}</h3><p>${esc(divisions[s.division][lang])} · ${esc(s.grades.join(' / '))}</p><p>${esc(s.rooms.join(' / '))}</p>${s.cancelled?`<span class="status">${T('已取消','Cancelled')}</span>`:s.changed?`<span class="status">${T('已变更','Updated')}</span>`:''}</button></label></article>`).join('')||`<p class="no-exams">${T('暂无考试','No exams')}</p>`}</section>`).join('')}</div>`;
...
async function showDetail(id){
 ...
 if(!school.user){$('#seating-content').innerHTML=`<p class="muted">${T('请使用学校账号登录后查看座位表。','Sign in with your school account to view seating.')}</p><a href="/api/school/login">Microsoft ${T('登录','sign-in')}</a>`;return;}

...
async function importFile(llm=false){
 ...
 $('#preview-title').textContent=llm?'LLM 座位表提取预览':'Excel 导入预览';$('#preview-content').innerHTML=`<p>读取 ${imported.length} 个座位；确认后替换本批次全部草稿座位，发布版本不变。</p><p class="warning">${(result.warnings||[]).map(esc).join('<br>')}</p><p class="import-errors">${esc(result.errors.join('\n'))}</p><div class="table-scroll"><table><thead><tr><th>考试</th><th>教室</th><th>排 / 列</th><th>学生</th></tr></thead><tbody>${imported.slice(0,100).map(s=>`<tr><td>${esc(draft.sessions.find(e=>e.id===s.examId)?.title||s.examId)}</td><td>${esc(s.room)}</td><td>${s.row} / ${s.column}</td><td>${esc(s.className)} ${esc(s.name)} ${esc(s.englishName)}</td></tr>`).join('')}</tbody></table></div><p class="muted">预览前 100 条；确认后导入全部有效记录。</p><button id="accept-import" class="primary" ${result.errors.length||!imported.length?'disabled':''}>确认替换草稿座位</button>`;$('#preview-dialog').showModal();$('#accept-import').onclick=()=>{if(draft!==target||draft.version!==version||dirty){notice('草稿已修改，请重新提取。');return;}draft.seats=imported;markDirty();$('#preview-dialog').close();edit();notice('已导入到编辑器，请保存草稿，再预览发布。',true);};

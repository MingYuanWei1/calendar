...
const kinds=()=>({off:T('放假','Holiday'),school:T('调休上课','School day'),default:T('恢复默认','Default')});
...
function render(main){
 const plans=app.data.plans,[ty,tm]=app.today.split('-').map(Number);
 ...
 for(let d=0;d<count;d++){
  const iso=addDays(start,d),plan=plans[iso],classes=['day'];
  ...
  const badge=plan?(plan.kind==='off'?T('休','Off'):T('上课','Class')):'';
  cells+=`<button type="button" class="${classes.join(' ')}" data-day="${iso}" aria-pressed="${iso>=view.selA&&iso<=view.selB}" aria-label="${esc(mdw(iso)+(plan?' · '+(plan.kind==='off'?T('放假','Holiday'):T('调休上课','School day'))+' '+tx(plan.title):''))}"><span class="top"><span class="n">${d+1}</span>${badge?`<span class="badge">${badge}</span>`:''}</span><span class="name">${esc(plan?tx(plan.title):'')}</span></button>`;
 }
 cells+='<span class="day blank"></span>'.repeat((7-(dow(start)+count)%7)%7);
 const ranges=planRanges(plans),named=view.kind!=='default';
 main.innerHTML=`<div class="page">
 <header class="page-head"><div><div class="kicker">${T('管理后台','Admin console')}</div><h1>${T('放假与调休','School days')}</h1><p class="sub">${T('全校适用，独立于事件和筛选。未设置时，周一至周五为工作日，周六日为普通周末。保存会覆盖所选日期的现有安排，不更改事件。','Applies school-wide, independently of events and filters. Dates default to weekdays or weekends. Saving replaces existing day settings in the selected range; events are unchanged.')}</p></div></header>
 <div class="days-grid"><div style="display:flex;flex-direction:column;gap:20px;min-width:0">
  <div class="legend"><span><i style="background:var(--color-bg)"></i>${T('工作日','Weekday')}</span><span><i style="background:var(--color-neutral-200)"></i>${T('周末','Weekend')}</span><span><i style="background:var(--color-accent-200);border-color:var(--color-accent-400)"></i>${T('放假','Holiday')}</span><span><i style="border:1px dashed var(--color-accent)"></i>${T('调休上课','Adjusted school day')}</span><span class="hint">${T('点击开始日期，再点击结束日期','Click a start date, then an end date')}</span></div>
 ...
 <div class="side-panel" style="padding:0;gap:22px">
  ...
  <section><div class="list-head">${T('已设置日期','Configured dates')}</div>${ranges.map((r,i)=>`<div class="range-row"><button type="button" data-range="${i}"><strong>${esc(rangeLabel(r.start,r.end))}</strong><small>${r.kind==='off'?kinds().off:kinds().school}${tx(r.title)?' · '+esc(tx(r.title)):''}</small></button><button type="button" class="btn btn-ghost" style="font-size:12px" data-restore="${i}">${T('恢复默认','Restore')}</button></div>`).join('')||`<p class="muted" style="font-size:13px;padding:10px 0">${T('暂无设置','No overrides yet')}</p>`}</section>
 </div></div></div>`;
 ...
 page.onclick=async e=>{
  ...
  else if(b.dataset.restore){const r=ranges[Number(b.dataset.restore)];await save(main,{start:r.start,end:r.end,kind:'default',title:['','']},T('已恢复默认。','Restored to defaults.'));return;}
  else if(b.hasAttribute('data-save')){await save(main,{start:view.selA,end:view.selB,kind:view.kind,title:view.kind==='default'?['','']:[view.zh.trim(),view.en.trim()]},T('已保存，公共校历已更新。','Saved. The public calendar is updated.'));return;}
  else return;
  render(main);

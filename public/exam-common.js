...
export function seatingMarkup(batch,seating,exam,roomName,point,lang=0){
 const room=seating.rooms.find(r=>r.name===roomName);if(!room)return '<p>座位表尚未发布 / Not published</p>';
 const active=batch.sessions.filter(s=>!s.cancelled&&s.date===exam.date&&s.rooms.includes(roomName)&&s.start<=point&&point<s.end);
 ...
 for(let r=1;r<=room.rows;r++){
  ...
  for(let c=1;c<=room.columns;c++){
   ...
   cells+=`<div class="seat${seat?'':' empty'}">${seat?`<small>${esc(seat.className)}</small><strong>${esc(seat.name)}</strong><span>${esc(seat.englishName)}</span><small>${esc(lang?(s.titleEn||s.title):s.title)}</small>`:(lang?'Empty':'空位')}</div>`;
  }
 }

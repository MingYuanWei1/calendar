...
export function seatingMarkup(batch,seating,exam,roomName,point,lang=0,editable=false){
 ...
 for(let r=1;r<=room.rows;r++){
  ...
  for(let c=1;c<=room.columns;c++){
   ...
   cells+=`<${tag}${attributes} class="seat${seat?'':' empty'}">${seat?`<small>${esc(seat.className)}</small><strong>${esc(seat.name)}</strong><span>${esc(seat.englishName)}</span><small>${esc(lang?(s.titleEn||s.title):s.title)}</small>`:(lang?'Empty':'空位')}</${tag}>`;
  }
 }

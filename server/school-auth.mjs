...
function returnPath(value,origin){
 try{
  ...
  if(url.origin!==origin||!['/','/index.html','/exams.html','/exams','/exams-admin.html','/accounts.html'].includes(url.pathname))return '/';
...
export function installSchoolAuth(app,db,{origin,tenantId='',clientId='',clientSecret='',loginUrl='',preview=false,localUser=()=>null}){
 ...
 const user=req=>{const identityUser=identity(req);if(!identityUser)return null;const account=schoolAccount(db,identityUser.id,identityUser.name);return account.disabled?null:{id:account.id,name:account.name,role:account.role};};
